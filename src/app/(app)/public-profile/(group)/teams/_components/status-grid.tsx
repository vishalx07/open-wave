import { Avatar, Card, CardContent, Text } from "@jamsr-ui/react";
import { STATUS_DATA } from "../data";
import { FreePremiumStatusGrid } from "./free-premium-status-gird";

export const StatusGrid = () => {
  return (
    <div className="mb-5 grid grid-cols-2 gap-5 lg:grid-cols-4">
      {STATUS_DATA.map((item) => {
        const { value, title, count, icon, users } = item;

        if (value === "free") return null;

        if (value === "premium") return <FreePremiumStatusGrid key={value} />;
